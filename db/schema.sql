...
-- ============================================================
-- ARQUITECTURA DE PROCESOS: cerrar el ciclo, causa raíz y competencia
-- verificada del equipo (ILUO). Ver db/migrate_add_process_architecture.sql
-- ============================================================

-- Causa raíz de una respuesta del Agente de IA que escaló o que el
...
-- material no conforme, etc.
ALTER TABLE chat_feedback ADD COLUMN IF NOT EXISTS root_cause VARCHAR(40);

-- Cerrar el ciclo real: cuando Calidad resuelve una pregunta que escaló
...
-- Autonomía ligada a competencia verificada (marco ILUO): I = en inducción,
-- L = en aprendizaje supervisado, U = capacitado para trabajar sin
-- supervisión, O = puede entrenar a otros. Roster completo del equipo de un
-- proceso, adicional al nombre asignado por nivel en
-- autonomy_role_assignments.
...
-- ============================================================
-- MÓDULO "CONTROL CALIDAD": formularios de inspección embebidos (Microsoft
-- Forms) y tablero de tareas del equipo de Calidad.
-- Ver db/migrate_add_control_calidad.sql
-- ============================================================

-- Formularios de inspección — el enlace guardado debe ser el de "Insertar
-- código" (embed) de Microsoft Forms, no el enlace normal de compartir.
...
-- Vista de Registros opcional (Excel Online / SharePoint) — los datos
-- reales de las respuestas nunca se guardan aquí, solo la URL de embed.
...
-- directamente desde la tarjeta pública de Control Calidad.
...
-- Convierte cada PDF a Markdown (texto + visión de Gemini en páginas con
-- poco texto) antes de indexarlo. Ver db/migrate_add_knowledge_base.sql
-- ============================================================
...
-- ============================================================
-- Inspecciones en Campo (ver db/migrate_add_field_inspections.sql para
-- el detalle explicado)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS field_inspections (
  ...
  cant_total NUMERIC(10,3) NOT NULL DEFAULT 0,
  cant_retenida NUMERIC(10,3) NOT NULL DEFAULT 0,
...
-- Metrología Pro — Reposición y Baja (ver db/migrate_add_metrology_replacements.sql)
