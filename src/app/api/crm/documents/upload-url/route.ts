 ...
 * lleguen al código de la ruta, así que un PDF grande nunca llegaría vivo
 * a un POST normal. El token que devuelve Storage solo autoriza subir a
 * esta ruta puntual, no da acceso general al bucket.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const docId = `pdf-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const storagePath = `${processSlug || 'general'}/${docId}.pdf`;

    const { data, error } = await supabase.storage.from(RAG_PDF_BUCKET).createSignedUploadUrl(storagePath);
    if (error || !data) {
